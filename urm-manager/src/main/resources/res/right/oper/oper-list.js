...
$(function(){
	...
	_dataTables = $('#operList').DataTable({
	  	"language": {
        ...
        "columns": [// 每页展示什么
             ...
             { 
            	 "data": "operId",
        		 "render": function (data, type, full, meta) {
                     return '<a href="'+ctx+'/oper/detail?operId='+data+'">'+full.operCode+'</a>';
                 },
            	 "bSortable": false
             },
             { "data": "operName" },
	...
	$(".checkall").click(function () {
		 if ($(this).prop("checked") === true) {
	            ...
	            $('#operList tbody tr').addClass('selected');
	     } else {
	            $(".checkchild").prop("checked", false);
	            $('#operList tbody tr').removeClass('selected');
	     }      
	});
    ...
    $("#btn-delAll").click(function(){
    	var checkRow = $('#operList tbody').find("input[name=deleteRow]:checked");
    	...
    	layer.msg('你确定删除账户['+checkRow.length+"]个账户", {
  		  ...
  		  ,yes: function(index){
  		    ...
  		    $.ajaxPostJson(ctx+"/oper/delete",{"deleteIds":deleteIds},function(ret){
  				_dataTables.draw(false);
  			})
  		  }
  	});
    ...
    $("#btn-export").click(function(){
    	...
    	$.download(ctx+"/oper/download",data);
    });
    ...
    $('#btn-edit-submit').click(function(){
    	if(editVali.form()){
    		...
    		var sendData = JSON.stringify(data);
    		console.log(sendData);
    		$.ajaxPostJson(ctx+"/oper/edit",sendData,function(ret){
    			addVali.resetForm();
    ...
    //修改
    $('#operList tbody').on('click', '#editRow', function () {
		var data = _dataTables.row($(this).parents('tr')).data();
		console.log(data);
		console.log(data.status);
		$("#editModal").find("input[name=operId]").val(data.operId);
		$("#editModal").find("input[name=operCode]").val(data.operCode);
		$("#editModal").find("input[name=operName]").val(data.operName);
		$("#editModal").find("select[name=status]").val(data.status);	
		$("#editModal").modal("show");
    });
    
    //单个删除
    $('#operList tbody').on('click', '#delRow', function () {
    	var data = _dataTables.row($(this).parents('tr')).data();
		var sendData = {"deleteIds":data.operId};
    	layer.msg('你确定删除账户['+data.operName+"]", {
    		  time: 0 //不自动关闭
    		  ,btn: ['确认', '取消']
    		  ,yes: function(index){
    		    layer.close(index);
    		    $.ajaxPostJson(ctx+"/oper/delete",sendData,function(ret){
    				_dataTables.draw(false);
    			})
    		  }
    	});
    ...
    $("#addForm").cxSelect({
    	...
    	selects: ['appId', 'firstFunctionId','secondFunctionId'],
    	jsonValue:"v"
    });
